...
export function AISearchInput(props: ComponentProps<'form'>) {
  ...
  return (
    <form
      {...props}
      className={cn('flex gap-2 items-end w-full min-w-0', props.className)}
      onSubmit={onStart}
    >
      <Input
        value={input}
...
function Input(props: ComponentProps<'textarea'>) {
  const shared = cn('col-start-1 row-start-1 w-full min-w-0', props.className);
  ...
  return (
    <div className="grid flex-1 min-w-0">
      <textarea
        ...
        rows={1}
        className={cn(
          'resize-none bg-transparent placeholder:text-fd-muted-foreground focus-visible:outline-none min-h-[2.5rem] overflow-hidden',
          shared,
        )}
      />
      <div className={cn(shared, 'break-words whitespace-pre-wrap invisible overflow-hidden')}>
...
export function AISearchTrigger({
  ...
  return (
    <button
      ...
      className={cn(
        position === 'float' && [
          'fixed bottom-4 gap-2 px-4 py-2.5 end-4 shadow-lg z-20 transition-[translate,opacity]',
      ...
      <Presence present={open}>
        <div
          className={cn(
            ...
            'max-lg:fixed max-lg:inset-x-4 max-lg:top-6 max-lg:bottom-6 max-lg:border max-lg:rounded-2xl max-lg:shadow-xl',
            'lg:fixed lg:top-0 lg:right-0 lg:h-dvh lg:border-s lg:shadow-xl',
            open
              ? 'animate-fd-dialog-in lg:animate-[ask-ai-open_200ms]'
              : 'animate-fd-dialog-out lg:animate-[ask-ai-close_200ms]',
          )}
        >
          <div className="flex flex-col size-full p-3 max-lg:max-h-[calc(100dvh-3rem)] lg:w-[var(--ai-chat-width)] min-h-0">
            <AISearchPanelHeader />
            <AISearchPanelList className="flex-1 min-h-0" />
            <div className="rounded-xl border bg-fd-secondary text-fd-secondary-foreground shadow-sm has-focus-visible:shadow-md overflow-hidden">
              <div className="p-2">
                <AISearchInput />
              </div>
